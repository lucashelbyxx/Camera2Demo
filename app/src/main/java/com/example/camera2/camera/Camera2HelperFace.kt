...
import com.example.camera2.toast
import com.example.camera2.util.BitmapUtils
...
class Camera2HelperFace(val mActivity: Activity, private val mTextureView: AutoFitTextureView) {
    private val TAG = Camera2HelperFace::class.java.simpleName
    ...
    private fun initCameraInfo() {
        ...
        log(TAG, "预览最优尺寸 ：${mPreviewSize.width} * ${mPreviewSize.height}, 比例  ${mPreviewSize.width.toFloat() / mPreviewSize.height}")
        log(TAG, "保存图片最优尺寸 ：${mSavePicSize.width} * ${mSavePicSize.height}, 比例  ${mSavePicSize.width.toFloat() / mSavePicSize.height}")
    ...
    private fun initFaceDetect() {
        ...
        log(TAG, "成像区域  ${activeArraySizeRect.width()}  ${activeArraySizeRect.height()} 比例: ${activeArraySizeRect.width().toFloat() / activeArraySizeRect.height()}")
        log(TAG, "预览区域  ${mPreviewSize.width}  ${mPreviewSize.height} 比例 ${mPreviewSize.width.toFloat() / mPreviewSize.height}")

        for (mode in faceDetectModes) {
            log(TAG, "支持的人脸检测模式 $mode")
        }
        log(TAG, "同时检测到人脸的数量 $faceDetectCount")
    }
    ...
    private fun handleFaces(result: TotalCaptureResult) {
        ...
        if (faces != null) {
            for (face in faces) {
                ...
                log(TAG, "原始人脸位置: ${bounds.width()} * ${bounds.height()}   ${bounds.left} ${bounds.top} ${bounds.right} ${bounds.bottom}   分数: ${face.score}")
                log(TAG, "转换后人脸位置: ${resultFaceRect.width()} * ${resultFaceRect.height()}   ${resultFaceRect.left} ${resultFaceRect.top} ${resultFaceRect.right} ${resultFaceRect.bottom}   分数: ${face.score}")
            }
        }
        ...
        log(TAG, "onCaptureCompleted  检测到 ${faces?.size} 张人脸")
    }
    ...
    private fun getBestSize(targetWidth: Int, targetHeight: Int, maxWidth: Int, maxHeight: Int, sizeList: List<Size>): Size {
        ...
        for (size in sizeList) {
            ...
            log(TAG, "系统支持的尺寸: ${size.width} * ${size.height} ,  比例 ：${size.width.toFloat() / size.height}")
        }

        log(TAG, "最大尺寸 ：$maxWidth * $maxHeight, 比例 ：${targetWidth.toFloat() / targetHeight}")
        log(TAG, "目标尺寸 ：$targetWidth * $targetHeight, 比例 ：${targetWidth.toFloat() / targetHeight}")
    ...
    private fun exchangeWidthAndHeight(displayRotation: Int, sensorOrientation: Int): Boolean {
        ...
        when (displayRotation) {
            ...
            else -> log(TAG, "Display rotation is invalid: $displayRotation")
        }

        log(TAG, "屏幕方向  $displayRotation")
        log(TAG, "相机方向  $sensorOrientation")
        return exchange
    }
    ...
    private fun configureTransform(viewWidth: Int, viewHeight: Int) {
        ...
        log(TAG, "configureTransform $viewWidth  $viewHeight")
    }
